...
const Movie: React.FC<Props> = ({ data, onClick, closeOnClick, dotsOnClick, isDotsClicked }) => {

    let infoBlock;
    if(isDotsClicked) {
        infoBlock = <div className={styles.infoBlock}>
            <Button onClick={closeOnClick} type="button" title="&#10005;" styleType="infoBlockClose"/>
            <Button onClick={onClick} id={data.id} type="button" title="edit" styleType="infoBlock"/>
            <Button onClick={onClick} id={data.id} type="button" title="delete" styleType="infoBlock"/>
        </div>
    } else {
